...
def distribute_and_over_or(s):
    ...
    if s.op == '|':
        ...
        conj = utils_lpw.find_if((lambda d: d.op == '&'), s.args)
        if not conj:
...
def pl_resolve(ci, cj):
    ...
    for di in disjuncts(ci):
        for dj in disjuncts(cj):
            if di == ~dj or ~di == dj:
                dnew = utils_lpw.unique(utils_lpw.removeall(di, disjuncts(ci)) + 
                              utils_lpw.removeall(dj, disjuncts(cj)))
...
def pl_fc_entails(KB, q):
    ...
    True
    """
    count = dict([(c, len(conjuncts(c.args[0]))) for c in KB.clauses if c.op == '>>'])
    inferred = utils_lpw.DefaultDict(False)
...
def occur_check(var, x):
    ...
    elif not isinstance(x, str) and utils_lpw.issequence(x):
        for xi in x:
...
### [Fig. 7.13]
alpha = expr("~P12")
to_cnf(Fig_7_13 & ~alpha)
#((~P12 | B11) & (~P21 | B11) & (P12 | P21 | ~B11) & ~B11 & P12)
tt_entails(Fig_7_13, alpha) 

pl_resolution(PropKB(Fig_7_13), alpha) 


### [Fig. 7.15]
pl_fc_entails(PropKB(Fig_7_13), expr('SomethingSilly')) 
